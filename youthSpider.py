...
class FkYouthStudy:
    ...
    def get_jpg(self, h):  # 切割图片
        if os.path.exists(sys.argv[0].strip("/youthSpider.py") + "/tmp"):
            for i in os.listdir(sys.argv[0].strip("/youthSpider.py") + "/tmp"):
                os.remove(sys.argv[0].strip("/youthSpider.py") + "/tmp/" + i)
        else:
            os.mkdir("./tmp")
        response = requests.get('https://h5.cyol.com/special/daxuexi/' + h + '/images/end.jpg', verify=False).content

        f = open("{}.jpg".format(sys.argv[0].strip("/youthSpider.py") + "/tmp/tmp1"), mode="wb")
        f.write(response)
        f.close()
        ...
        cv2.imencode('.jpg', image3)[1].tofile(sys.argv[0].strip("/youthSpider.py") + r'/tmp/{}.jpg'.format(self.title))
    ...
    def mail(self):
        ...
        try:
            ...
            fd = open("{}.jpg".format(sys.argv[0].strip("/youthSpider.py") + "/tmp/"+self.title), "rb")  # 读取本地图片
